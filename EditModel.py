...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    for i in range(1, len(word)+1):
      # The corrupted signal are this character
      corruptLetters = word[i - 1]
      # The correct signal is this character appended with an alphabet
      # Insert each of the 26 alphabets at each location
      for j in range(0, 26):
        correctLetters = corruptLetters + EditModel.ALPHABET[j]

        correction = "%s%s%s" % (word[1:i],correctLetters[1], word[i:])
  ...
  def transposeEdits(self, word):
    ...
    for i in range(0, len(word)-1):
      ...
      # The correct signal are this character and perceding character, switch place
  ...
  def replaceEdits(self, word):
    ...
    for i in range(0, len(word)):
      # The corrupted signal are this character
      corruptLetter = word[i:i+1]

      # The correct signal is an alphabet of the 26 for substitution
      for j in range(0, 26):
        correctLetter = EditModel.ALPHABET[j]
